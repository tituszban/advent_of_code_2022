...
def step(lines: list[str], iter_count: int = 10):
    ...
    points = list(zip(*area.nonzero()))
    x_range = max(x := [p[1] for p in points]) - min(x) + 1
    y_range = max(y := [p[0] for p in points]) - min(y) + 1

    return x_range * y_range - np.sum(area)


def main():
    print(step(example_input), 110)
