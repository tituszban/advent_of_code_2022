...
def a_star(grid: dict[Point, GridPoint], start: Point, end: Point):
    frontier: Frontier = Frontier(FrontierPoint(start, tuple(), 0, 0))
    ...
    while frontier.any():
        ...
        for neighbour in g.neighbours:
            if grid[neighbour].height - 1 <= g.height and neighbour not in explored and not frontier.has(neighbour):
...
def main():
    ...
    print(a_star(grid, start, end), 31)
